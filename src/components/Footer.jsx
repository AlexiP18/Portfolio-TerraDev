...
function Footer() {
  ...
  useEffect(() => {
    client.fetch(SITE_SETTINGS_QUERY)
      ...
      .catch(err => console.error("Error fetching site settings on Footer:", err));
  }, []);

  const email = settings?.contactEmail || 'hola@terraby.te';
  const location = settings?.location || 'Ambato, Ecuador';
  const title = settings?.title || 'Terrabyte EC';
  const description = settings?.description || 'Ingeniería desde los Andes.\\nSoftware de alto rendimiento para negocios que escalan.';
      ...
      <div className="max-w-6xl mx-auto px-6 pt-14 pb-10">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10">
          ...
          <div className="lg:col-span-1">
            ...
            {/* Slogan */}
            <p className="font-body text-sm text-text-muted leading-relaxed mt-3 max-w-[220px] whitespace-pre-line">
              {description}
            </p>
          ...
          <div>
            ...
            <ul className="space-y-3 list-none p-0 m-0">
              {dynamicContactInfo.map(({ icon, text, href }) => (
                <li key={text}>
                  <a
                    href={href}
      ...
      <div className="border-t border-white/[0.06]">
        <div className="max-w-6xl mx-auto px-6 py-5 flex flex-col sm:flex-row items-center justify-between gap-3">
          <p className="font-body text-xs text-text-muted text-center sm:text-left">
            © {new Date().getFullYear()} {title}. Hecho con código y café en Ambato.
          </p>
